export function showToast(msg, args?: { duration?: number, textClassName?: string, cb?: VoidCallback }) {
  ...
  $toast.className = "fixed left-2 right-2 top-1 z-999 flex items-center px-3 py-2 rounded-md border text-foreground bg-background text-base shadow shadow-shadow transition-all duration-300 ease-in-out transform -translate-y-10"
